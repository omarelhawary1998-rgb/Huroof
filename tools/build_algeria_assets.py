...
SRC = os.environ.get("ALG_SRC", "/tmp/algeria.jpg")
...
pr.save(P("print","events","algeria-born-print.jpg"), quality=95)
# 2. PREVIEW
prev = src.copy(); prev.thumbnail((1400,2100), Image.LANCZOS)
protected(prev).save(P("previews","events","algeria-born.jpg"), quality=90)
# 3. PIN
pin = src.copy(); pin.thumbnail((1000,1500), Image.LANCZOS)
pad(protected(pin),1000,1500).save(P("pins","events","algeria-born-pin.jpg"), quality=90)
...
framed(protected(src.copy().resize((600,int(600*H/W)),Image.LANCZOS)),
       P("mockups","events","algeria-born-framed.jpg"))
# 5. SOCIAL
S=lambda *a:P("social","events",*a)
pad(src,1080,1350).save(S("algeria-instagram-portrait-1080x1350.jpg"),quality=90)
cx=src.crop((0,(H-W)//2 if H>W else 0,W,(H-W)//2+W if H>W else H)).resize((1080,1080),Image.LANCZOS)
cx.save(S("algeria-instagram-square-1080.jpg"),quality=90)
pad(src,1080,1920).save(S("algeria-story-1080x1920.jpg"),quality=90)
pad(src,1200,675).save(S("algeria-facebook-1200x675.jpg"),quality=90)
print("source",src.size)
for r,_,fs in os.walk(P()):
    for fn in fs:
        if "algeria" in fn: pth=os.path.join(r,fn); print(" ",os.path.relpath(pth,ROOT),Image.open(pth).size)
