...
SRC = os.environ.get("BRA_SRC", "/tmp/brazil_a.jpg")
...
pr.save(P("print","events","brazil-champions-print.jpg"), quality=95)
# 2. PREVIEW
prev = src.copy(); prev.thumbnail((1400,2100), Image.LANCZOS)
protected(prev).save(P("previews","events","brazil-champions.jpg"), quality=90)
# 3. PIN
pin = src.copy(); pin.thumbnail((1000,1500), Image.LANCZOS)
pad(protected(pin),1000,1500).save(P("pins","events","brazil-champions-pin.jpg"), quality=90)
...
framed(protected(src.copy().resize((600,int(600*H/W)),Image.LANCZOS)),
       P("mockups","events","brazil-champions-framed.jpg"))
# 5. SOCIAL
S=lambda *a:P("social","events",*a)
pad(src,1080,1350).save(S("brazil-instagram-portrait-1080x1350.jpg"),quality=90)
cx=src.crop((0,(H-W)//2 if H>W else 0,W,(H-W)//2+W if H>W else H)).resize((1080,1080),Image.LANCZOS)
cx.save(S("brazil-instagram-square-1080.jpg"),quality=90)
pad(src,1080,1920).save(S("brazil-story-1080x1920.jpg"),quality=90)
pad(src,1200,675).save(S("brazil-facebook-1200x675.jpg"),quality=90)
print("source",src.size)
for r,_,fs in os.walk(P()):
    for fn in fs:
        if "brazil" in fn: pth=os.path.join(r,fn); print(" ",os.path.relpath(pth,ROOT),Image.open(pth).size)
