...
SRC = os.environ.get("FRA_SRC", "/tmp/france_b.jpg")
...
pr.save(P("print","events","france-glory-print.jpg"), quality=95)
# 2. PREVIEW
prev = src.copy(); prev.thumbnail((1400,2100), Image.LANCZOS)
protected(prev).save(P("previews","events","france-glory.jpg"), quality=90)
# 3. PIN
pin = src.copy(); pin.thumbnail((1000,1500), Image.LANCZOS)
pad(protected(pin),1000,1500).save(P("pins","events","france-glory-pin.jpg"), quality=90)
...
framed(protected(src.copy().resize((600,int(600*H/W)),Image.LANCZOS)),
       P("mockups","events","france-glory-framed.jpg"))
# 5. SOCIAL
S=lambda *a:P("social","events",*a)
pad(src,1080,1350).save(S("france-instagram-portrait-1080x1350.jpg"),quality=90)
cx=src.crop((0,(H-W)//2 if H>W else 0,W,(H-W)//2+W if H>W else H)).resize((1080,1080),Image.LANCZOS)
cx.save(S("france-instagram-square-1080.jpg"),quality=90)
pad(src,1080,1920).save(S("france-story-1080x1920.jpg"),quality=90)
pad(src,1200,675).save(S("france-facebook-1200x675.jpg"),quality=90)
print("source",src.size)
for r,_,fs in os.walk(P()):
    for fn in fs:
        if "france" in fn: pth=os.path.join(r,fn); print(" ",os.path.relpath(pth,ROOT),Image.open(pth).size)
